...
class MetricsPrintingFedAvg(fl.server.strategy.FedAvg):
    def aggregate_fit(self, server_round, results, failures):
        ...
        if aggregated_parameters is not None:
            ...
            checkpoint_path = f"global_model_round_{server_round}.pkl"
            with open(checkpoint_path, "wb") as f:
                pickle.dump(ndarrays, f)
            print(f"\n [CHECKPOINT] Aggregated parameters for Round {server_round} saved to '{checkpoint_path}'")
...
def fit_config(server_round: int):
    ...
    Fetches 10 synthetic lures (5 clean, 5 phishing), tokenizes them, and ships them via gRPC.
    """
    print(f"\n[ gRPC Link ] Preparing IID transport payload for Round {server_round}...")
    
    try:
        # Generate 10 balanced samples (5 benign, 5 phishing)
        raw_payload = generate_synthetic_lures(round_num=server_round, count=10)
        
        # Convert to numerical tokens
        tokenized_payload = tokenize_payloads(raw_payload)
        
        # Compress structural JSON array into raw bytes
        serialized_lessons = json.dumps(tokenized_payload).encode('utf-8')
        
        print(f"[ gRPC Link ] Payload compression complete. Streaming 10-sample package over subnet...")
...
def main():
    print("=====================================================")
    print("[ Server ] Initializing Tier 2 IID Baseline Workstation Server...")
    print("=====================================================")
    ...
    # ==========================================
    # AUTO-EXPORT TO DISTINCT CSV
    # ==========================================
    print("\n[ Server ] Training complete. Exporting IID run history...")
    try:
        import pandas as pd
        ...
        df.to_csv("iid_baseline_metrics.csv", index=False)
        print("=======> SUCCESS: Saved IID metrics to 'iid_baseline_metrics.csv'")
